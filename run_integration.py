...
def calculate_path(src, dst):
    ...
    return pp.run_path_planning(step,
                                    start=(src[0], src[1]),
                                    finish=(dst[0], dst[1]), algo='theta*', heur='manhattan',
                                    show_grid=True)
...
def generate_task_list(filepath):
    ...
    for line in open(filepath):
        if re.match(r'^\(\w(\w|_)*(\ *(\w(\w|_)*)?)*\)', re.sub(r"(\d(\.|\d)*: ) *", "", line)):
            ...
            if tasks[-1] == 'MOVEFAST' or tasks[-1] == 'MOVESLOW':
...
def visualize_paths_from_pddl(task_plan, path_to_map):
    position = (5, 5) # change this to the starting position for your PDDL problem
    ...
    for task in task_plan: # Iterate the whole task list
        ...
        if type(task) is tuple: # Movement must be a tuple, so we are only interested in this case
            if task[0] == 'MOVEFAST' or task[0] == 'MOVESLOW': # This is the action we were looking for
...
task_plan = generate_task_list('../res/planningplanetary.txt') # Must be a path to a text file
visualize_paths_from_pddl(task_plan, '../res/mapa.png') # path_to_map_file should be a path to an image